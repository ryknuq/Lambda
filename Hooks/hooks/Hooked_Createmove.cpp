...
#include "..\..\features\lagcompensation\anti_exploit.h"
...
void __stdcall hooks::hooked_createmove(int sequence_number, float input_sample_frametime, bool active, bool& send_packet)
{
	...
	if (m_pcmd->m_buttons & IN_ATTACK2 && cfg.ragebot.enable && g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER)
		m_pcmd->m_buttons &= ~IN_ATTACK2;

	if (cfg.ragebot.enable && !g_ctx.globals.weapon->can_fire(true))
	{
	...
	g_ctx.globals.current_weapon = g_ctx.globals.weapon->get_weapon_group(cfg.ragebot.enable);
	...
	if (cfg.misc.crouch_in_air && !(g_ctx.local()->m_fFlags() & FL_ONGROUND))
