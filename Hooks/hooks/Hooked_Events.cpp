...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	if (!strcmp(event->GetName(), crypt_str("player_footstep")))
	{
		...
		if (e->valid(false, false))
		{
			...
			if (cfg.player.type[type].footsteps)
			{
				...
				info.m_flLife = 2.0f;
				info.m_flWidth = (float)cfg.player.type[type].thickness;
				info.m_flFadeLength = 1.0f;
				info.m_flAmplitude = 0.0f;
				info.m_flRed = (float)cfg.player.type[type].footsteps_color.r();
				info.m_flGreen = (float)cfg.player.type[type].footsteps_color.g();
				info.m_flBlue = (float)cfg.player.type[type].footsteps_color.b();
				info.m_flBrightness = (float)cfg.player.type[type].footsteps_color.a();
				info.m_flSpeed = 0.0f;
				...
				info.m_flEndRadius = (float)cfg.player.type[type].radius;
	...
	else if (!strcmp(event_name, crypt_str("weapon_fire")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if (current_shot)
			{
				...
				++g_ctx.globals.total_shots_fired;  // Track total shots for session
			}
		}
	}
	else if (!strcmp(event_name, crypt_str("bullet_impact")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if (cfg.esp.server_bullet_impacts)
				m_debugoverlay()->BoxOverlay(position, Vector(-2.0f, -2.0f, -2.0f), Vector(2.0f, 2.0f, 2.0f), QAngle(0.0f, 0.0f, 0.0f), cfg.esp.server_bullet_impacts_color.r(), cfg.esp.server_bullet_impacts_color.g(), cfg.esp.server_bullet_impacts_color.b(), cfg.esp.server_bullet_impacts_color.a(), 4.0f);
			...
			if (current_shot && aim::get().last_target[current_shot->last_target].record.player->valid(true, false))
			{
				...
				auto target_animstate = aim::get().last_target[current_shot->last_target].record.player->get_animation_state();
				if (target_animstate)
				{
					current_shot->shot_info.target_animation_sequence = aim::get().last_target[current_shot->last_target].record.player->get_animlayers()[1].m_nSequence;
					current_shot->shot_info.target_animation_cycle = aim::get().last_target[current_shot->last_target].record.player->get_animlayers()[1].m_flCycle;
				}
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			otheresp::get().damage_marker[user_id].hurt_color = cfg.esp.damage_marker_color;
	...
	{
		if (!cfg.misc.show_default_log && eventlogs::get().last_log && cfg.misc.log_output[EVENTLOG_OUTPUT_CONSOLE])
		{
	...
	if ((cfg.esp.bullet_tracer || cfg.esp.enemy_bullet_tracer) && cfg.player.enable)
