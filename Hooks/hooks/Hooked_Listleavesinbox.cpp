...
int __fastcall hooks::hooked_listleavesinbox(void* ecx, void* edx, Vector& mins, Vector& maxs, unsigned short* list, int list_max)
{
	...
	if (!cfg.player.type[ENEMY].chams[PLAYER_CHAMS_VISIBLE] && !cfg.player.type[TEAM].chams[PLAYER_CHAMS_VISIBLE] && !cfg.player.type[LOCAL].chams[PLAYER_CHAMS_VISIBLE] && !cfg.player.fake_chams_enable && !cfg.player.backtrack_chams)
		return original_fn(ecx, mins, maxs, list, list_max);
