...
void nightmode::modulate(MaterialHandle_t i, IMaterial* material, bool backup = false) 
{
	...
	if (strstr(name, crypt_str("World")))
	{
		...
		material->AlphaModulate((float)cfg.esp.world_color.a() / 255.0f);
		material->ColorModulate((float)cfg.esp.world_color.r() / 255.0f, (float)cfg.esp.world_color.g() / 255.0f, (float)cfg.esp.world_color.b() / 255.0f);
	}
	else if (strstr(name, crypt_str("StaticProp")))
	{
		...
		material->AlphaModulate((float)cfg.esp.props_color.a() / 255.0f);
		material->ColorModulate((float)cfg.esp.props_color.r() / 255.0f, (float)cfg.esp.props_color.g() / 255.0f, (float)cfg.esp.props_color.b() / 255.0f);
	}
}
