...
void worldesp::skybox_changer()
{
	...
	case 23:
		skybox_name = cfg.esp.custom_skybox;
		break;
	}
	...
	if (skybox_number != cfg.esp.skybox)
	{
		changed = true;
		skybox_number = cfg.esp.skybox;
	}
	...
	else if (color_r != cfg.esp.skybox_color[0])
	{
		changed = true;
		color_r = cfg.esp.skybox_color[0];
	}
	else if (color_g != cfg.esp.skybox_color[1])
	{
		changed = true;
		color_g = cfg.esp.skybox_color[1];
	}
	else if (color_b != cfg.esp.skybox_color[2])
	{
		changed = true;
		color_b = cfg.esp.skybox_color[2];
	}
	...
	if (changed)
	{
		...
		for (auto i = materialsystem->FirstMaterial(); i != materialsystem->InvalidMaterial(); i = materialsystem->NextMaterial(i))
		{
			...
			if (strstr(material->GetTextureGroupName(), crypt_str("SkyBox")))
				material->ColorModulate(cfg.esp.skybox_color[0] / 255.0f, cfg.esp.skybox_color[1] / 255.0f, cfg.esp.skybox_color[2] / 255.0f);
		}
	}
}
...
void worldesp::fog_changer()
{
	...
	if (fog_end->GetInt() != cfg.esp.fog_distance)
		fog_end->SetValue(cfg.esp.fog_distance);

	static auto fog_maxdensity = m_cvar()->FindVar(crypt_str("fog_maxdensity"));

	if (fog_maxdensity->GetFloat() != (float)cfg.esp.fog_density * 0.01f)
		fog_maxdensity->SetValue((float)cfg.esp.fog_density * 0.01f);

	char buffer_color[12];
	sprintf_s(buffer_color, 12, "%i %i %i", cfg.esp.fog_color.r(), cfg.esp.fog_color.g(), cfg.esp.fog_color.b());
...
void worldesp::world_modulation(entity_t* entity)
{
	...
	entity->set_m_flCustomBloomScale(cfg.esp.bloom * 0.01f);

	entity->set_m_bUseCustomAutoExposureMin(TRUE);
	entity->set_m_flCustomAutoExposureMin(cfg.esp.exposure * 0.001f);

	entity->set_m_bUseCustomAutoExposureMax(TRUE);
	entity->set_m_flCustomAutoExposureMax(cfg.esp.exposure * 0.001f);
}
...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	auto col = cfg.esp.molotov_timer_color;
...
void worldesp::smoke_timer(entity_t* entity)
{
	...
	auto col = cfg.esp.smoke_timer_color;
	g_Render->DrawRing3D1(origin.x, origin.y, origin.z, 150, 360, Color(col.r(), col.g(), col.b(), 255), Color(col.r(), col.g(), col.b(), 35), 2, factor, true);
	Vector screen_origin;
...
void worldesp::grenade_projectiles(entity_t* entity)
{
		...
		client_class->m_ClassID == CBaseCSGrenadeProjectile || client_class->m_ClassID == CDecoyProjectile || client_class->m_ClassID == CMolotovProjectile)
	{
		...
		{
			if (cfg.esp.grenade_esp[GRENADE_BOX])
			{
				g_Render->Rect(box.x, box.y, box.w, box.h, cfg.esp.grenade_box_color);
				g_Render->Rect(box.x - 1, box.y - 1, box.w + 2, box.h + 2, Color(0, 0, 0, 80));
				g_Render->Rect(box.x + 1, box.y + 1, box.w - 2, box.h - 2, Color(0, 0, 0, 80));

				if (cfg.esp.grenade_esp[GRENADE_ICON])
					g_Render->DrawString(box.x + box.w / 2, box.y - 21, cfg.esp.projectiles_color, render2::centered_x | render2::outline, c_menu::get().weapons, grenade_icon.c_str());

				if (cfg.esp.grenade_esp[GRENADE_TEXT])
					g_Render->DrawString(box.x + box.w / 2, box.y + box.h + 2, cfg.esp.projectiles_color, render2::centered_x | render2::outline, c_menu::get().OpenSans, grenade_name.c_str());
			}
			else
			{
				if (cfg.esp.grenade_esp[GRENADE_ICON] && cfg.esp.grenade_esp[GRENADE_TEXT])
				{
					g_Render->DrawString(box.x + box.w / 2, box.y + box.h / 2 - 10, cfg.esp.projectiles_color, render2::centered_x | render2::outline, c_menu::get().weapons, grenade_icon.c_str());
					g_Render->DrawString(box.x + box.w / 2, box.y + box.h / 2 + 7, cfg.esp.projectiles_color, render2::centered_x | render2::outline, c_menu::get().OpenSans, grenade_name.c_str());
				}
				else
				{
					if (cfg.esp.grenade_esp[GRENADE_ICON])
						g_Render->DrawString(box.x + box.w / 2, box.y + box.h / 2, cfg.esp.projectiles_color, render2::centered_x | render2::outline, c_menu::get().weapons, grenade_icon.c_str());
...
{
	if (!cfg.esp.grenade_proximity_warning)
		return;
...
void worldesp::bomb_timer(entity_t* entity)
{
	...
	g_Render->DrawString(19, height - factor, Color(255, 255, 255, 255), render2::none, c_menu::get().OpenSans, damage_text.c_str());
	...
	if (math::WorldToScreen(entity->GetAbsOrigin(), screen))
	{
		if (bomb->m_hBombDefuser() > 0)
		{
			...
			{
				g_Render->DrawString(screen.x, screen.y - 75, Color(25, 80, 153, 200), render2::centered_x, c_menu::get().weapons, "r");
				g_Render->DrawString2(screen.x - 7, screen.y - 55, Color(232, 232, 232, 200), render2::none, c_menu::get().OpenSans, std::to_string((int)(round(c4_defuse_time))) + " s");
				arc(screen.x, screen.y - 50, 30, 32, -90, 360 * defuse_value, Color(25, 80, 153, 200));
			}
			else
			{
				g_Render->DrawString(screen.x, screen.y - 75, Color(25, 80, 153, 200), render2::centered_x, c_menu::get().weapons, "r");
				g_Render->DrawString2(screen.x - 7, screen.y - 55, Color(232, 232, 232, 200), render2::none, c_menu::get().OpenSans, "NO TIME");
				arc(screen.x, screen.y - 50, 30, 32, -90, 360 * defuse_value, Color(25, 80, 153, 200));
			}
		}
		else
		{
			g_Render->DrawString(screen.x, screen.y - 75, Color(255, 70, 70, 200), render2::centered_x, c_menu::get().weapons, "o");
			g_Render->DrawString2(screen.x - 7, screen.y - 55, Color(232, 232, 232, 200), render2::none, c_menu::get().OpenSans, std::to_string((int)(round(bomb_timer))) + " s");
...
void worldesp::dropped_weapons(entity_t* entity)
{
	...
	if (util::get_bbox(weapon, box, false))
	{
		...
		{
			g_Render->Rect(box.x, box.y, box.w, box.h, cfg.esp.box_color);
			g_Render->Rect(box.x - 1, box.y - 1, box.w + 2, box.h + 2, Color(0, 0, 0, 80));
			g_Render->Rect(box.x + 1, box.y + 1, box.w - 2, box.h - 2, Color(0, 0, 0, 80));

			if (cfg.esp.weapon[WEAPON_ICON])
			{
				g_Render->DrawString(box.x + box.w / 2, box.y + box.h + 20, cfg.esp.weapon_color, render2::centered_x | render2::outline, c_menu::get().weapons, weapon->get_icon());
				offset = 14;
			}

			if (cfg.esp.weapon[WEAPON_TEXT])
				g_Render->DrawString(box.x + box.w / 2, box.y + box.h + 10, cfg.esp.weapon_color, render2::centered_x | render2::outline, c_menu::get().OpenSans, weapon->get_name().c_str());

			if (cfg.esp.weapon[WEAPON_AMMO] && entity->GetClientClass()->m_ClassID != CBaseCSGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSmokeGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSensorGrenadeProjectile && entity->GetClientClass()->m_ClassID != CMolotovProjectile && entity->GetClientClass()->m_ClassID != CDecoyProjectile)
			{
				...
				g_Render->FilledRect(box.x, box.y + box.h + 6, weapon->m_iClip1() * box.w / m_weaponsystem()->GetWeaponData(weapon->m_iItemDefinitionIndex())->iMaxClip1, 2, cfg.esp.weapon_ammo_color);
			}
		...
		else
		{
			...
			if (cfg.esp.weapon[WEAPON_AMMO] && entity->GetClientClass()->m_ClassID != CBaseCSGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSmokeGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSensorGrenadeProjectile && entity->GetClientClass()->m_ClassID != CMolotovProjectile && entity->GetClientClass()->m_ClassID != CDecoyProjectile)
			{
				...
				if (cfg.esp.weapon[WEAPON_ICON] && cfg.esp.weapon[WEAPON_TEXT])
					pos = 19;
				else if (cfg.esp.weapon[WEAPON_ICON])
					pos = 8;
				else if (cfg.esp.weapon[WEAPON_TEXT])
					pos = 19;
				...
				g_Render->FilledRect(box.x, box.y + box.h / 2 + pos, weapon->m_iClip1() * box.w / m_weaponsystem()->GetWeaponData(weapon->m_iItemDefinitionIndex())->iMaxClip1, 2, cfg.esp.weapon_ammo_color);
			}
