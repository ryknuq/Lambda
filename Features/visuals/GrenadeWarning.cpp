...
void DrawBeamPaw(Vector src, Vector end, Color color)
{
	...
	beamInfo.m_flWidth = cfg.esp.proximity_tracers_width;
	beamInfo.m_flEndWidth = cfg.esp.proximity_tracers_width;
...
bool c_grenade_prediction::data_t::draw() const
{
	...
	for (auto i = 1u; i < m_path.size(); ++i) {
		...
		{
			auto color = cfg.esp.grenade_proximity_warning_progress_color;
			if (i % 4)
	...
	auto col = cfg.esp.grenade_proximity_warning_progress_color;
	Ray_t ray;
	...
	if (trace.fraction < 1.0f && m_index == 46) {
		...
		Vector adw;
		if (math::WorldToScreen(vvt, adw))
		g_Render->CircleFilled(adw.x, adw.y - g_size.y * 0.5f - 12, 21, Color(25, 25, 25, col.a()), 60);
		g_Render->CircleFilled(adw.x, adw.y - g_size.y * 0.5f - 12, 21, Color(200, 25, 25, alpha_damage), 60);
		g_Render->two_sided_arc(adw.x, adw.y - g_size.y * 0.5f - 12, 20, percent, Color(col.r(), col.g(), col.b(), 255), 3);
		g_Render->DrawString(adw.x, adw.y - g_size.y * 0.5f - 12, Color::White, render2::centered_x | render2::centered_y, c_menu::get().weapons, index_to_grenade_name_icon(m_index));
	}
	else {
		...
		g_Render->DrawString(prev_screen.x, prev_screen.y - g_size.y * 0.5f - 12, Color::White, render2::centered_x | render2::centered_y, c_menu::get().weapons, index_to_grenade_name_icon(m_index));
	}
	...
	if (cfg.esp.grenade_proximity_warning && !isOnScreen(vEnemyOrigin, screenPos))
	{
		...
		g_Render->DrawString(warn.x, warn.y - (size * 3) * 0.5f, Color::White, render2::centered_x | render2::centered_y, c_menu::get().weapons, index_to_grenade_name_icon(m_index));
		g_Render->TriangleFilled(points.at(0).x, points.at(0).y, points.at(1).x, points.at(1).y, points.at(2).x, points.at(2).y, Color(cfg.esp.grenade_proximity_warning_progress_color));
...
void c_grenade_prediction::grenade_warning(projectile_t* entity)
{
	...
	if (entity->IsDormant() || !cfg.esp.grenade_proximity_warning)
		return;
