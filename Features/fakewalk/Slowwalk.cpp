...
void slowwalk::autostop(CUserCmd* m_pcmd, float custom_speed)
{
	...
	auto modifier = custom_speed == -1.0f ? (1.0f - cfg.misc.slowwalk_speed / 100.0f) * 0.06f + 0.02f : custom_speed;
...
void slowwalk::create_move(CUserCmd* m_pcmd, float custom_speed)
{
	...
	auto modifier = custom_speed == -1.0f ? (1.0f - cfg.misc.slowwalk_speed / 100.0f) * 0.2f + 0.1f : custom_speed;
	auto max_speed = modifier * (g_ctx.globals.scoped ? weapon_info->flMaxPlayerSpeedAlt : weapon_info->flMaxPlayerSpeed);
