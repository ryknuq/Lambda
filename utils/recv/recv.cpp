...
static void viewModelSequence(recvProxyData& data, void* arg2, void* arg3) noexcept
{
	if (g_ctx.local())
	{
		if (const auto activeWeapon = g_ctx.local()->m_hActiveWeapon())
		{
			...
			else if (cfg.skins.rare_animations && activeWeapon->m_iItemDefinitionIndex() == WEAPON_DEAGLE && data.value._int == 7)
				data.value._int = 8;
			else if (cfg.skins.rare_animations && activeWeapon->m_iItemDefinitionIndex() == WEAPON_KNIFE_FALCHION && data.value._int == 12)
				data.value._int = 13;
		}
	}
...
{
	data.value._int = cfg.ragebot.enable;
