...
void CMenu::SetupUI() {
	...
	config.misc.miscellaneous.trashtalk = misc->AddCheckBox("Trashtalk");
	config.misc.miscellaneous.trashtalk_events = misc->AddMultiCombo("Trashtalk on", { "Kill", "Death" });
	...
	config.misc.miscellaneous.trashtalk->SetCallback([]() {
		config.misc.miscellaneous.trashtalk_events->SetVisible(config.misc.miscellaneous.trashtalk->get());
	});

	sound_preview_blocked = false;
}