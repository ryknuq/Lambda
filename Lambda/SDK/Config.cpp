...
void on_load() {
    ...
    const std::string file_path = std::filesystem::current_path().string() + "/lambda/" + name + ".cfg";
    if (!std::filesystem::exists(file_path)) {
        Utils::Print("[lambda] config doesnt exists: %s\n", name);
        return;
    }
...
void CConfig::Init() {
    ...
    add(config.misc.miscellaneous.trashtalk);
    add(config.misc.miscellaneous.trashtalk_events);
