...
#define NET_LAMBDA_CODE 1099264u

enum LambdaMsg_Type : unsigned char {
	LambdaMsg_SharedESP
};
...
typedef void(*tRecieveLambdaDataCallback)(const SharedVoiceData_t*);

class CNetMessages {
	std::vector<tRecieveVoiceDataCallback> m_voiceDataCallbacks;
	std::vector<tRecieveLambdaDataCallback> m_lambdaDataCallbacks;
...
public:
	...
	void AddLambdaDataCallback(tRecieveLambdaDataCallback callback) {
		m_lambdaDataCallbacks.emplace_back(callback);
	}
	...
	void RemoveLambdaDataCallback(tRecieveLambdaDataCallback callback) {
		for (auto it = m_lambdaDataCallbacks.begin(); it != m_lambdaDataCallbacks.end(); it++) {
			if (*it == callback) {
				m_lambdaDataCallbacks.erase(it);
				return;
			}
		}
	}
